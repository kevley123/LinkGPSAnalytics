...
export default function MapaVivo() {
  ...
  const fetchLocation = useCallback(async (vehId: number) => {
    ...
    setLoadingLoc(true);
    try {
      const res = await fetch(`${API_BASE}/api/analytics/mi_ubicacion?vehiculo_id=${vehId}`, {
        headers: { 'Authorization': `Bearer ${authToken}`, 'Accept': 'application/json' },
      });
                 ...
                 <div className="flex -space-x-2">
                   {vehicles.slice(0, 3).map(v => (
                     <button 
                      ...
                      className={`w-8 h-8 rounded-full border-2 border-brand-dark flex items-center justify-center transition-transform hover:scale-110 ${
                        vehSel?.id === v.id ? 'bg-brand-orange text-white' : 'bg-brand-dark-3 text-neutral-500'
                      }`}
                     >
                        <Car size={12} />
                     </button>
